...
- **`recon_motion.py`** — fastMRI 形式マルチコイル k-space(`.h5`) → 画像再構成（PNG / DICOM 出力）
...
fastMRI 形式のマルチコイル k-space（`.h5`）を画像に再構成し、**PNG** または **DICOM** で
...
| `both` | PNG と DICOM の両方 |

ファイルがフォルダ直下にフラットに並ぶ場合は `<out_root>/<basename>/slNN.*`、
`motion/` のように `inter-scan_motion/…` のサブフォルダがある場合はその階層を保ったまま出力する。
...
# PNG と DICOM の両方、先頭2ファイルだけ動作確認
python recon_motion.py --in-root motion --out-root out_check --format both --limit 2
```
...
| `--format` | `png` | `png` / `dicom` / `both` |
| `--limit` | `0` | 先頭 N ファイルのみ（0=全部、動作確認用） |

### 注意

- k-space（`.h5`）と再構成画像（PNG/DICOM）は **患者データ**のため git に push しない
  （`.gitignore` で `motion*/`・`multicoil_test*/`・`gre_data*/`・`*.h5` を除外済み）。
- DICOM の絶対位置（IPP の原点）は元の患者座標が不明なため **FOV 中心を原点**として再構成している。
  同一ボリューム内のスライス間隔・厚み・面内スケールは header の実値で正しい。
- 必要パッケージ: `h5py`, `Pillow`（PNG）, `pydicom`（DICOM）。
