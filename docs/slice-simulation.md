...
![スライスプロファイル積分](images/slice_profile.png)

*左: 矢状断に重ねた 5mm 厚スライス窓（赤）。右2枚: 薄スライス1mm（1枚）と、矩形プロファイルで
隣接スライスを積分した厚スライス5mm。through-plane で構造が平均化され、実2D厚スライスに近づく。*
...
![出力面の選択](images/orientation.png)

*同一3Dボリュームを軸位/冠状/矢状にリスライスしてから厚スライス化する。等方3Dならどの面でも生成できる。*
...
![プロファイルの違い](images/profiles.png)

*左: rect（MRI第一近似）と gaussian（CT相当）のスライス感度プロファイル。右: それぞれで厚スライス化した結果（rectの方がシャープ）。*
