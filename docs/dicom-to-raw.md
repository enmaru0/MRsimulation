...
- **画素は既定で RescaleSlope/Intercept を適用した実値（HU/信号）** を 2byte で保存
  （`real = 格納値 × RescaleSlope + RescaleIntercept` をスライスごとに適用）。
  値域に応じて符号(int16/uint16)を自動判定し、int16範囲を超える大きさのときだけ
  10のべき乗で縮小する（`raw_rescale_slope` を CSV に記録、`real = raw × raw_rescale_slope`。
  通常域は `raw_rescale_slope = 1` ＝ raw が実値そのもの）。
  `--no-rescale` を付けると **DICOM 格納値のまま**（rescale 未適用）保存。
...
b 値は機種によって格納先が違うため、以下を**横断的に**探す:
標準 `(0018,9087)` → `MRDiffusionSequence (0018,9117)` 内 → **Siemens `(0019,100C)`**
→ **Philips `(2001,1003)`** → **GE `(0043,1039)`**（先頭値、+1e9 オフセット除去）。
標準タグが空でも私的タグから b 値を見つけて分割できる。

#### 位置リピートによる安全網

それでも分割タグが見つからない場合でも、**同一スライス位置が N 回現れる＝N ボリューム**と
判断して分割する（ファイル名は `_v0`, `_v1`, … 警告を表示）。これにより、未知のタグ構成でも
DWI 等は確実に分離される。

- 分割が起きた時だけファイル名に**サフィックス**が付く（例
  `PatientID_5_20230101_130000_b1000.raw`）。通常シリーズ（タグが一定・位置が一意）は
  分割されず従来どおり 1 ボリューム。
- CSV に `split_label` / `diffusion_b_value` / `volume_in_series` 列を記録。
- 想定外の機種で b 値が拾えない時は `--split-tags` でタグを明示追加できる:
  ```bash
...
| `--no-rescale` | off | RescaleSlope/Intercept を適用せず DICOM 格納値のまま出力（既定は適用） |
...
- 既定では raw が**既に実値**（DICOM の `RescaleSlope/Intercept` 適用済み）。さらに int16範囲
  超で縮小がかかった場合のみ `実値 = raw × raw_rescale_slope`（CSV列）。元の DICOM
  `RescaleSlope/Intercept` も参照用に CSV に残す。`--no-rescale` 時は raw=格納値で、
  実値は `格納値 × RescaleSlope + RescaleIntercept`。
