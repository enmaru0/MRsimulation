...
![再構成パイプライン](images/recon_pipeline.png)

*k空間を中心化IFFTしてコイルごとの画像にし、root-sum-of-squares で合成する（合成ファントムによる図解）。*
...
![アンダーサンプリングによるボケ](images/undersampling.png)

*フルサンプリング（左）に対し、R=5・R=10 のゼロ詰め再構成は高周波が欠落してボケ・リンギングが増える。*
...
![DC位置と四隅アーチファクト](images/kspace_dc.png)

*端DC（標準FFT配置）のデータを中心化IFFTすると像が半周ずれて四隅に分裂する（左）。`--kspace-dc corner` で正しく中央に再構成（右）。*
...
### 3D ボリュームから任意厚・任意間隔の 2D 厚スライスを合成

真の 3D（等方）取得を再構成すれば、そこから **2D 厚スライス撮像をシミュレート**できる
（スライス方向にスライスプロファイル積分）。2通り:

**(A) recon_motion 一発（整数 mm・取得スライス方向）** — `--slab`。1mm 等方なら厚= N mm:

```bash
# CC(1mm等方) を 5mm厚 / 6mm間隔 の 2D に（rectプロファイル合成）
python recon_motion.py --in-root <CC_12ch> --out-root cc_2d --format dicom \
    --real-imag-axis -1 --transpose 0,3,1,2 --kspace-dc corner \
    --pixel-spacing 1 --slice-spacing 1 --slice-thickness 1 \
    --slab 5 --slab-step 6
```

**(B) [`mri_slice_sim.py`](slice-simulation.md)（任意 mm＋面選択 AX/COR/SAG＋プロファイル）** —
まず 3D を DICOM 化し、それを入力に厚スライス合成（**非整数厚や別断面**もOK）:

```bash
# 1) 3D を 1mm 等方 DICOM に再構成
python recon_motion.py --in-root <CC_12ch> --out-root cc_dicom --format dicom \
    --real-imag-axis -1 --transpose 0,3,1,2 --kspace-dc corner \
    --pixel-spacing 1 --slice-spacing 1 --slice-thickness 1
# 2) 任意厚・任意間隔・任意断面の 2D 厚スライスを合成（例 4.5mm厚/7mm間隔・軸位）
python mri_slice_sim.py cc_dicom/<basename> out_2d \
    --thickness 4.5 --spacing 7 --profile rect --orientation axial --pattern "*.dcm"
```

両方とも合成3Dデータで検証済み（厚み/間隔タグが指定どおりになることを確認）。物理モデルは
magnitude・矩形プロファイル（[mri_slice_sim の方針](slice-simulation.md#本ツールが対象とするケース)）。
