...
------
    ...
    # 両方・先頭2ファイルだけ試す
    python recon_motion.py --in-root motion --out-root out --format both --limit 2
...
def reconstruct(kspace: np.ndarray) -> np.ndarray:
    """(S,C,H,W) complex -> (S,H,W) magnitude（コイル RSS）。"""
    coil_imgs = ifft2c(kspace)
    return np.sqrt((np.abs(coil_imgs) ** 2).sum(axis=1))
...
def process_file(path: str, in_root: str, out_root: str, fmt: str) -> int:
    ...
    if fmt in ("dicom", "both"):
        meta = parse_header(raw_hdr)
        save_dicom(rss, vmax, meta, os.path.basename(base), pid, acq, out_dir)

    print(f"[ok] {rel}  ({acq})  {rss.shape[0]} slices {rss.shape[1]}x{rss.shape[2]}  [{fmt}]")
    return rss.shape[0]
...
def main() -> None:
    ...
    ap.add_argument("--format", choices=["png", "dicom", "both"], default="png",
                    help="出力形式（既定 png）")
    ...
    print(f"[start] {len(files)} files -> {args.out_root}  format={args.format}")

    n_files = n_slices = 0
    for p in files:
        try:
            n_slices += process_file(p, args.in_root, args.out_root, args.format)
            n_files += 1
