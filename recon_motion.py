...
- binary : <out_root>/<basename>.raw/.hdr/.tag（3D ボリュームを 2byte 生バイナリ）
           ...
           .hdr = 'X Y Z 2 dx dy dz '、.tag = 各種メタ情報
- both   : png + dicom
...
def save_binary(vol: np.ndarray, vmax: float, meta: dict, basename: str,
                patient_id: str, acquisition: str, out_base: str,
    ...
    オーバーフロー対策: 2byte を符号付き short(int16) として読むビューア向けに、
    最大値を 32000（int16範囲 ±32767 内）へスケールする。スケールは各ボリュームの
    実最大値基準なので、低磁場処理(acq-matrix/ノイズ)で値が増えても飽和しない。
    ...
    SMAX = 32000                                 # int16 安全範囲（±32767）に収める
    data_max = float(np.max(vol))                # 実データ最大（飽和を防ぐ）
    scale = SMAX / (data_max + 1e-9)
    stored = np.clip(np.round(vol * scale), 0, SMAX).astype("<i2")  # signed int16 (z,y,x)
...
def process_file(path: str, in_root: str, out_root: str, fmts: set,
    ...
    if "binary" in fmts:
        save_binary(rss, vmax, meta, os.path.basename(base), pid, acq,
                    os.path.join(out_root, base), flip_y=raw_flip_y)
